...
class StackVisualizer:
   def __init__(self, root):
       ...
       # Make canvas
       self.stack_canvas = Canvas(self.window, width=self.canvas_width, height=self.canvas_height, bg="chocolate",
                                  relief=RAISED, bd=10)
   ...
   def push_element(self):  # Push button action
       if len(self.last_label_value_keep) == 6:
           ...
           self.add_btn = Button(self.window, text="Add", font=("Arial", 10, "bold"), bg="blue", fg="red",
                                 relief=RAISED, bd=3, padx=3, pady=3, command=lambda: self.make_block('<Return>'))
   ...
   def make_block(self, e):  # Element containing block making
       try:
           ...
           self.block_make = self.stack_canvas.create_rectangle(self.block_left, self.block_up, self.block_right,
                                                                self.block_down, fill="black", width=2, outline="blue")
           self.entry_number = Label(self.stack_canvas, textvar=self.number, bg="black", fg="red",
                                     font=("Arial", 11, "bold"))
   ...
   def push_data(self):  # Element entry process in stack container
       try:
           ...
           while self.number_set_x < 265:
               ...
               self.block_right += 2
               self.block_make = self.stack_canvas.create_rectangle(self.block_left, self.block_up, self.block_right,
                                                                    self.block_down, fill="black", width=2,
                                                                    outline="blue")
               self.entry_number.place(x=self.number_set_x, y=self.number_set_y)
               self.window.update()
   ...
   def reset_with_position_set(self):  # Reset the coordinate value
       ...
       self.final_set_block = Label(self.window, text=self.value_entry.get(), bg="black", fg="red",
                                    font=("Arial", 11, "bold"))
